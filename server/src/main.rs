...
async fn main_multiplayer() -> anyhow::Result<()> {
    ...
    {
        let state_for_status = std::sync::Arc::clone(&state);
        tokio::spawn(async move {
            let mut ticker = tokio::time::interval(std::time::Duration::from_secs(5));
            loop {
                ticker.tick().await;
                let (count, addrs): (usize, Vec<std::net::SocketAddr>) = {
                    let st = state_for_status.lock();
                    (st.players.len(), st.addr_by_player.values().copied().collect())
                };
                println!("[status] players={} peers={:?}", count, addrs);
            }
        });
    }
    ...
    loop {
        ...
        match protocol::decode_client(data) {
            Ok(protocol::ClientToServer::Join(join)) => {
                ...
                println!("[join] player_id={} username='{}' addr={}", pid, info.username, addr);
            }
            ...
            Err(err) => {
                println!("[decode_error] from {}: {}", addr, err);
